...
const TORSO_GEOMETRY = new THREE.CylinderGeometry(0.53, 0.45, 1, 7, 2)
...
const FINGER_GEOMETRY = new THREE.CapsuleGeometry(0.035, 0.1, 2, 5)
const HAIR_GEOMETRY = new THREE.TetrahedronGeometry(0.5, 0)
const NOSE_GEOMETRY = new THREE.ConeGeometry(0.065, 0.19, 5)
const EAR_GEOMETRY = new THREE.SphereGeometry(0.5, 6, 4)
const EYE_GEOMETRY = new THREE.SphereGeometry(0.5, 7, 4)
const LINE_GEOMETRY = new THREE.CapsuleGeometry(0.035, 0.5, 2, 5)
const CUFF_GEOMETRY = new THREE.CylinderGeometry(0.17, 0.16, 0.095, 7)
const COLLAR_GEOMETRY = new THREE.CylinderGeometry(0.43, 0.4, 0.34, 7, 1, true)
const COLLAR_RIM_GEOMETRY = new THREE.TorusGeometry(0.415, 0.025, 4, 7)
const SHIRT_PANEL_GEOMETRY = new RoundedBoxGeometry(0.52, 0.76, 0.035, 2, 0.035)
const POCKET_GEOMETRY = new RoundedBoxGeometry(0.2, 0.27, 0.035, 1, 0.025)
const BOOT_GEOMETRY = new RoundedBoxGeometry(0.39, 0.25, 0.59, 1, 0.055)
const SOLE_GEOMETRY = new RoundedBoxGeometry(0.42, 0.085, 0.63, 1, 0.025)
...
function PlayerHand({ material, mirror = false }: { material: THREE.Material; mirror?: boolean }) {
  return (
    <group>
      <mesh geometry={HAND_GEOMETRY} scale={[0.22, 0.25, 0.18]} material={material} castShadow />
      {[-0.07, 0, 0.07].map((x, index) => (
        <mesh
          key={x}
          geometry={FINGER_GEOMETRY}
          position={[x, -0.17 - index * 0.008, 0.035]}
          rotation={[0.35, 0, (mirror ? -1 : 1) * (x * 1.2)]}
          scale={[0.8, 1 - index * 0.08, 0.85]}
          material={material}
          castShadow
        />
      ))}
...
export function PlayerModel({ animation, surge, damageFlash }: PlayerModelProps) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (animation === 'WALK' || animation === 'RUN') {
      ...
      weaponZ = -0.12 + phase * 0.04
    } else if (animation.startsWith('ATTACK')) {
      const duration = 0.5 / (surge ? 1.18 : 1)
      ...
      if (animation === 'ATTACK_1') {
        ...
        weaponZ = stagedValue(progress, -0.08, -0.42, 0.32)
        ...
        weaponZ = stagedValue(progress, -0.08, 0.28, -0.35, 0.3, 0.57)
      } else {
        ...
        weaponZ = stagedValue(progress, -0.08, -0.18, 0.2, 0.38, 0.66)
      }
      headY = -torsoY * 0.22
      headX = torsoX * -0.18
    } else if (animation === 'HEAVY') {
      const duration = 0.86 / (surge ? 1.18 : 1)
      const progress = Math.min(1, time / duration)
      rightArmX = stagedValue(progress, -0.18, -2.82, 0.76, 0.48, 0.7)
      rightArmZ = stagedValue(progress, -0.09, -0.4, 0.3, 0.48, 0.7)
      rightElbowX = stagedValue(progress, -0.3, -0.5, 0.2, 0.48, 0.7)
      leftArmX = stagedValue(progress, 0.08, -2.18, 0.55, 0.48, 0.7)
      leftArmZ = stagedValue(progress, 0.07, 0.46, 0.18, 0.48, 0.7)
      leftElbowX = stagedValue(progress, -0.08, -0.5, -0.08, 0.48, 0.7)
      torsoX = stagedValue(progress, 0, -0.24, 0.55, 0.48, 0.7)
      torsoY = stagedValue(progress, 0, -0.32, 0.38, 0.48, 0.7)
      rootY = stagedValue(progress, 0, 0.04, -0.12, 0.48, 0.7)
      weaponZ = stagedValue(progress, -0.08, -0.28, 0.26, 0.48, 0.7)
      headX = stagedValue(progress, 0.025, -0.14, 0.16, 0.48, 0.7)
    } else if (animation === 'DODGE') {
      const progress = Math.min(1, time / 0.3)
      const roll = Math.sin(progress * Math.PI)
      rootX = roll * 0.44
      rootZ = Math.sin(progress * Math.PI * 2) * 0.1
      torsoZ = roll * 0.18
      leftArmX = 0.72
      rightArmX = 0.62
      leftElbowX = -0.45
    ...
    if (leftForearm.current) leftForearm.current.rotation.x = damp(leftForearm.current.rotation.x, leftElbowX, delta, 17)
    if (rightForearm.current) rightForearm.current.rotation.x = damp(rightForearm.current.rotation.x, rightElbowX, delta, 17)
    ...
    materials.trim.emissiveIntensity = surge
    ...
    <group ref={root} dispose={null}>
      <group ref={torso} position={[0, 1.56, 0]}>
        ...
        <mesh
          ...
          material={materials.trim}
        />
        <mesh geometry={PATCH_GEOMETRY} position={[-0.27, 0.15, -0.347]} rotation={[0, Math.PI, 0.25]} scale={[0.13, 0.22, 1]} material={materials.jacketWear} />
        <mesh geometry={PATCH_GEOMETRY} position={[0.3, -0.05, -0.349]} rotation={[0, Math.PI, -0.3]} scale={[0.1, 0.18, 1]} material={materials.purple} />
        <mesh geometry={LINE_GEOMETRY} position={[0, 0.25, -0.352]} scale={[0.42, 0.38, 0.42]} material={materials.purple} />

        <group ref={leftArm} position={[-0.56, 0.37, 0]}>
          <mesh geometry={JOINT_GEOMETRY} scale={[0.29, 0.29, 0.3]} material={materials.jacket} castShadow />
        ...
        <group ref={rightArm} position={[0.56, 0.37, 0]}>
          ...
          <group ref={rightForearm} position={[0, -0.55, 0]}>
            ...
            <mesh geometry={CUFF_GEOMETRY} position={[0, -0.49, 0]} material={materials.trim} />
            <group position={[0, -0.59, 0.015]} rotation={[0.08, 0, -0.04]}>
              <PlayerHand material={materials.skin} />
              <group ref={weaponMount} rotation={[0, 0, -0.08]}>
                <Weapon
                  trail={attacking}
                  heavy={animation === 'HEAVY'}
                  surge={surge}
      ...
      <group ref={head} position={[0, 2.49, 0.015]}>
        <mesh geometry={HEAD_GEOMETRY} scale={[0.63, 0.77, 0.6]} material={materials.skin} castShadow />
        <mesh geometry={HEAD_GEOMETRY} position={[0, 0.2, -0.055]} scale={[0.64, 0.46, 0.62]} material={materials.hair} castShadow />
        <mesh geometry={JAW_GEOMETRY} position={[0, -0.19, 0.025]} scale={[0.5, 0.38, 0.5]} material={materials.skinDark} castShadow />
        <mesh geometry={EAR_GEOMETRY} position={[-0.34, -0.01, -0.01]} scale={[0.12, 0.2, 0.1]} material={materials.skinDark} />
        <mesh geometry={EAR_GEOMETRY} position={[0.34, -0.01, -0.01]} scale={[0.12, 0.2, 0.1]} material={materials.skinDark} />
